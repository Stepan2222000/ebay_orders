    ...
    <div className={styles.backdrop} data-testid="detail-modal" onClick={onClose}>
      ...
      {retryConfirm ? (
        <ConfirmDialog
          ...
          body={
            data?.order_number
              ? `Заказ #${data.order_number} будет удалён вместе со всеми его данными (товары, возвраты, трек-номера). Скриншоты этого заказа вернутся в очередь сборки — попросите агента собрать его заново в чате. Этот снимок будет распознан повторно.`
